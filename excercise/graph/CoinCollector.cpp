...
ll f(ll v, vector<ll> &cant_c, vector<ll> &dp, vector<vector<ll>> &g2){
    if(SZ(g2[v])==0) return cant_c[v];
    if(dp[v] != -1) return dp[v];
    ll act = cant_c[v];
    for(auto u : g2[v]){
        dp[v] = max(act + f(u, cant_c, dp, g2), dp[v]);
    }
    return dp[v];
}
...
int main(){
    ...
    while(t--){
        ...
        vector<ll> cant_c(SZ(colors));
        fore(i,0,n){
            cant_c[cmp[i]] += coins[i];
        }
        ...
        ll res = 0;
        vector<ll> dp(SZ(colors),-1);

        for(auto c : colors){
            res = max(res, f(c,cant_c,dp,g2));
        }
        cout << res << "\n";
