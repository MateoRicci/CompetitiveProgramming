...
double f(int r, int x){
    ...
    if(n-r==1)return (double)1;
    if(x==1)return y-r==1?(double)1:(double)0;
    double dx=x,dn=n-r;
    if(dp[r][x]!=(double)-1)return dp[r][x];
    double meVoy = (2.000/dn) * (dx-1.0000)/(dn-1.000);
    double seVaUnoMejor = ((dx-1.000)/dn) * ((dx-2.000)/(dn-1.000));
    double seVaUnoPeor = 1-meVoy-seVaUnoMejor;
    // DGB(y);DGB(r);DGB(x);DGB(meVoy);DGB(seVaUnoMejor);DGB(seVaUnoPeor);RAYA;
    if(y==n-r){
        // cout << "ME VOY\n";
        dp[r][x] = meVoy;
    }
